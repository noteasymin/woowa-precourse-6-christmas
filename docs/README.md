...
- [ ] 주문
  - [ ] 주문 주의사항
    ...
    - [ ] 음료만 주문 시 주문 X
      - [ ] 음료로만 이루어져 있는지 체크
      - [ ] 음료로만 이루어져 있을 시 재입력 요구
    - [ ] 메뉴는 최대 20개까지 주문 가능
      - [ ] 메뉴 개수 카운트
      - [ ] 메뉴 개수 20개 넘을 시 재입력 요구
- [ ] 할인
  - [ ] 크리스마스 디데이 할인(2023.12.1 ~ 2023.12.25)
    - [ ] 1일 1,000원 할인으로 시작하여 1일 증가할 때마다 100원씩 증가 (25일 3,400원 할인)
  ...
  - [ ] 주말 할인(금요일, 토요일)
    ...
    - [ ] 메인 메뉴 개수 카운트
  - [ ] 특별 할인
    - [ ] 별이 있는 날짜에 총 주문 금액에서 1,000원 할인 (3일, 10일, 17일, 24일, 25일, 31일)
  - [ ] 증정 이벤트 할인
    - [ ] 총 주문 금액이 12만 원 이상일 때 샴페인 1개 증정 (25,000원)
  - [ ] 크리스마스 디데이 할인(25일까지)을 제외한 이벤트 31일까지 적용
...
- [ ] 출력
  - [ ] 주문 시 출력
    - [ ] `12월 중 식당 예상 방문 날짜는 언제인가요? (숫자만 입력해 주세요!)` 출력
      - [ ] 숫자는 1~31 사이의 숫자만 허용
      - [ ] 그 외에는 `[ERROR] 유효하지 않은 날짜입니다. 다시 입력해 주세요.` 출력
    - [ ] `주문하실 메뉴와 개수를 알려주세요.(e.g. 해산물파스타-2,레드와인-1,초코케이크-1)` 출력
      - [ ] 메뉴판에 없는 메뉴 입력 시 `[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.` 출력
      - [ ] 메뉴 형식이 다를 경우 `[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.` 출력
      - [ ] 중복 메뉴 입력했을 경우 `[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요.` 출력
      - [ ] 모든 예외 입력에는 재입력 요구
      - [ ] 주문 성공 시 출력은 자유롭게
  - [ ] 주문 끝났을 시 출력
    - [ ] 주문 메뉴 출력
